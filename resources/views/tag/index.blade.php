                                ...
                                <li class="list-group-item">
                                    ...
                                    @can('delete', $tag)
                                        <form style="display: inline;" action="/tag/{{ $tag->id }}" method="post">
                                            @csrf
                                            @method('DELETE')
                                            <input class="btn btn-outline-danger btn-sm ml-2 float-right" type="submit"
                                                   value="Delete">
                                        </form>
                                    @endcan
                                    <a class="btn btn-sm btn-light float-right mx-2" href="/hobby/tag/{{ $tag->id }}"><u>Used {{ $tag->hobbies->count() }} times</u></a>
                                </li>
                            @endforeach
                        </ul>
                        @can('create',$tag)
